...
struct queue_entry {
  ...
  // @FUN
  double sig_score;   /* Significance score computed by accumulating fs vals */
  u8    *func_flags;    /* Executed functions. Design for initial test cases */
...
enum {
  ...
  // @FUN: Add before `RARE` due to `create_alias_table()`.
  /* 07 */ FUN,     /* Function-significance-based power scheduling */
...
// @FUN
typedef struct fun_globals {

  u8  on;                     /* Whether fun_mode is on                   */
  u8  fuzz_started;           /* Does the main fuzz loop have kicked off? */
  u8 *tmpdir;                 /* Path to AFL_FUN_TEMP directory           */
  u32 func_num;               /* The number of functions                  */

  // FS updating

  u64   last_fs_update_time;    /* Last time point of updating FS values  */
  u64   fs_update_period;       /* Period of updating FS values           */

  // SHMs for call counts

  u32   cc_len;               /* Length of call counts SHMs                 */
  u32   cc_shm_size;          /* Size of call counts SHMs                   */
  u32   cc_flags_size;        /* Size of the area used for function flags   */
  u64  *cc_shm[SHM_IDX];      /* Call counts SHM between fuzzer and the PUT */

  // SHMs for FS

  u8      fs_idx_hot;         /* Idx of the fs_shm for writing  */
  u8      fs_idx_cld;         /* Idx of the fs_shm for reading  */
  u8      fs_static;          /* Whether using static FS        */
  u32     fs_len;             /* Length of FS SHM               */
  u32     fs_shm_size;        /* Size of FS SHM                 */
  double *fs_shm[SHM_IDX];    /* FS SHMs between fuzzer and DCC */

  // DCC subprocess

  pid_t dcc_pid;              /* Process pid          */
  FILE *dcc_infp;             /* Pipe to send shm_id  */

  // FUN scheduling

  u64     total_entries;             /* Number of seen entries                */
  double  min_score;                 /* The minimum sig_score ever seen       */
  double  max_score;                 /* The maximum sig_score ever seen       */
  double  total_sig_score;           /* Total sig_score achieved by test cases*/
...
typedef struct afl_state {
  ...
  // SHM is part of `afl_state`. Details are declared in type `sharedmem_t`.
  sharedmem_t      shm;
  ...
  // `limit_time_sig` determines whether using MOpt mode in:
  // (1) afl-fuzz-one.c: `fuzz_one()`;
  // (2) afl-fuzz-queue.c: `calculate_score()`;
  // (3) afl-fuzz-mutators.c: `setup_custom_mutators()`;
  ...
  u8 *in_dir,                           /* Input directory with test cases  */
      ...
      *use_banner,              /* Display banner (path to the fuzz target) */
  ...
  u8 schedule;                          /* Power schedule (default: FAST)   */
  u8 havoc_max_mult;
...
#define N_FUZZ_SIZE (1 << 21)
  ...
  // @FUN: total_bitmap_size - total coverage;
  //       total_bitmap_entries - total number of the test cases have coverage.
  u64 total_bitmap_size,                /* Total bit count for all bitmaps  */
      total_bitmap_entries;             /* Number of bitmaps counted        */

  // @FUN
  u32 min_bitmap_size, max_bitmap_size;
...
// @FUN
void   compute_sig_score_for_queue(afl_state_t *);
double adjust_fast_factor(afl_state_t *, struct queue_entry *, double);
double adjust_fun_factor(afl_state_t *, struct queue_entry *, double);
...
// @FUN
void dcc_update_fs_vals(afl_state_t *, u8);
void ck_dcc_status(afl_state_t *, u8 *);
...
// @FUN
void    fun_init(afl_state_t *, u8 *);
void    fun_deinit(afl_state_t *);
u32     get_func_num(u8 *tmpdir);
double *read_static_fs(fun_globals_t *);
