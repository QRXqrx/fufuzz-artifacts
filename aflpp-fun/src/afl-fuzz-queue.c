...
inline double min_max_norm(
    afl_state_t *afl, double val, double min, double max) {
  if (unlikely(val > max || val < min || max < min)) {
    fun_deinit(afl);
    FATAL("@FUN, invalid norm, val %lf, min %lf, max %lf", val, min, max);
  }
  double gap = max - min;
  return (gap > 0) ? ((val - min) / gap) : 0.0;
}


inline void fun_update_with_qentry(fun_globals_t *fun, struct queue_entry *q) {

  // Update min-max score
  fun->min_score =
      (q->sig_score < fun->min_score) ? q->sig_score : fun->min_score;
  fun->max_score =
      (q->sig_score > fun->max_score) ? q->sig_score : fun->max_score;

  // Update total

  fun->total_sig_score += q->sig_score;
  ++fun->total_entries;

}


/// Compute FS score for a test case.
double fun_compute_sig_score(fun_globals_t *fun) {

  double score = 0.0;

  if (fun->on) {

    // Compute FS score by accumulating FS values of the executed functions.
    for (u32 i = 1; i <= fun->func_num; ++i) {

      // We compute sig_score with (1) cold fs_shm, which provides a bit
      // outdated but steady fs values, and (2) hot cc_shm, which records
      // just-in-time functions' execution statuses.
      score += fun->fs_shm[fun->fs_idx_cld][i] * (double) fun->cc_shm[CC_IDX_HOT][i];

    }

  }

  if (unlikely(fun->logmode)) {
    fprintf(fun->log_fp, "fun_compute_sig_score(), min_score %lf, max_score %lf, q_score %lf\n",
            fun->min_score, fun->max_score, score);
  }
...
// @FUN: Except the lots of things AFL has done, we also need to compute FS
//       score for the saved test case under FUN mode
void add_to_queue(afl_state_t *afl, u8 *fname, u32 len, u8 passed_det) {
  ...
  if (fun->on && fun->fuzz_started) {

    // Normally, hot cc_shm should have been written or the dcc process will die.
    ck_dcc_status(afl, "add_to_queue()");

    // Compute sig_score and update fun status
    q->sig_score = fun_compute_sig_score(fun);
    fun_update_with_qentry(fun, q);
...
void cull_queue(afl_state_t *afl) {
  ...
  /* Let's see if anything in the bitmap isn't captured in temp_v.
     If yes, and if it has an afl->top_rated[] contender, let's use it. */
...
u32 calculate_score(afl_state_t *afl, struct queue_entry *q) {
  ...
  if (likely(afl->schedule < RARE) && likely(!afl->fixed_seed)) {
    ...
    // Adjust `perf_score` according to execution time.
  ...
  // Adjust `perf_score` according to bitmap_size
  ...
  // Adjust `perf_score` according to queue cycles.
  if (q->handicap >= 4) {
  ...
  // Adjust `perf_score` according to depth
  switch (q->depth) {
  ...
  switch (afl->schedule) {
    ...
    case FAST:
      ...
      factor = adjust_fast_factor(afl, q, factor);
    ...
    case FUN:
      ...
      // Also don't modify perf_score for unfuzzed seeds under FUN scheduling
      if (!q->fuzz_level) break;

      factor = adjust_fun_factor(afl, q, factor);
  ...
  // Why `unlikely` is used here?
  // if (unlikely(afl->schedule >= EXPLOIT && afl->schedule <= QUAD)) {
  if (unlikely(afl->schedule >= EXPLOIT && afl->schedule <= FUN)) {
...
// @FUN: Wrap as a function.
double adjust_fast_factor(
    afl_state_t *afl, struct queue_entry *q, double orig_fac) {

  double factor = orig_fac;
...
/// norm(mean(x)) = mean(norm(x)), est_coe = mean(norm(x)) / mean(norm(y))
inline double compute_est_coe(afl_state_t *afl) {

  fun_globals_t *fun = &afl->fun;

  u32    mean_x;
  double mean_y, mean_norm_x, mean_norm_y;

  mean_x = afl->total_bitmap_size / afl->total_bitmap_entries;
  mean_y = fun->total_sig_score / (double) fun->total_entries;

  mean_norm_x = min_max_norm(afl, mean_x, afl->min_bitmap_size, afl->max_bitmap_size);
  mean_norm_y = min_max_norm(afl, mean_y, fun->min_score, fun->max_score);

  if (unlikely(fun->logmode)) {
    fprintf(fun->log_fp, "compute_est_coe(), mean_x %u, mean_y %lf, mean_norm_x %lf, mean_norm_y %lf\n",
            mean_x, mean_y, mean_norm_x, mean_norm_y);
  }

  return (mean_norm_y / mean_norm_x);

}

inline double compute_scale_fun(double val, double est_val) {

  if (est_val == 0) return 0.2;

  double scale_fun, est_err, rho = 2.0;

  // Compute relative estimation error, est_err > -1
  est_err = (val / est_val) - 1;

  // Compute scale_fun
  scale_fun = 1 + rho * est_err;

  // Let scale_fun range in [0.25, 2.0]
  if (scale_fun < 0.2) scale_fun = 0.2;
  if (scale_fun > 2.0) scale_fun = 2.0;

  return scale_fun;

}

/// A Estimation based scheduling with a linear model: y = ax,
/// where a = mean(y) / mean(x). We reward seeds that are above estimation
/// while punish the seeds that are under estimation.
double adjust_fun_factor(
    afl_state_t *afl, struct queue_entry *q, double orig_fac) {

  fun_globals_t  *fun = &afl->fun;
  double          factor, fac_base, scale_fun,
                  norm_sig, norm_bitmap, est_coe, est_norm_sig;

  // Use fun factor as base
  fac_base = adjust_fast_factor(afl, q, orig_fac);

  // Compute norm values
  norm_bitmap = min_max_norm(afl, q->bitmap_size, afl->min_bitmap_size, afl->max_bitmap_size);
  norm_sig    = min_max_norm(afl, q->sig_score, fun->min_score, fun->max_score);

  // Estimate qscore with est_coe and q->bitmap_size.
  est_coe       = compute_est_coe(afl);
  est_norm_sig  = est_coe * norm_bitmap;

  // Compute scale factor using real value and estimated value
  scale_fun = compute_scale_fun(norm_sig, est_norm_sig);

  // Scale fac_base (factor-fast).
  factor = fac_base * scale_fun;

  // Sanitize
  if (unlikely(q->sig_score < fun->min_score)) {
    fun_deinit(afl);
    FATAL("@FUN, q_score(%lf) < min_score(%lf) with '%s'",
          q->sig_score, fun->min_score, q->fname);
  }

  if (unlikely(fun->logmode)) {
    fprintf(fun->log_fp, "adjust_fun_factor(), fac_base %lf, est_coe %lf, est_norm_sig %lf, norm_sig %lf, scale_fun %lf, factor %lf\n",
            fac_base, est_coe, est_norm_sig, norm_sig, scale_fun, factor);
  }
...
// @FUN
/// Compute sig_scores for entries in the queue using current fs values
void compute_sig_score_for_queue(afl_state_t *afl) {
  ...
  for (u32 idx = 0; idx < afl->queued_items; idx++) {
    ...
    FUN_LOG("Compute sig_score for '%s'...", fn);

    // Sanitize
    if (!q->func_flags)
      FATAL("@FUN, compute_sig_score_for_queue(), initial test case has NULL func_flags!!!");

    // Compute sig_scores. Also read from cold.
    q->sig_score = 0.0;
    for (u32 i = 1; i < fun->fs_len; ++i)
      q->sig_score += q->func_flags[i] * fun->fs_shm[fun->fs_idx_cld][i];

    // Log tail
    SAYF(cGRA "      sig_score = %lf\n" cRST, q->sig_score);

    // Update min and max scores.
    fun_update_with_qentry(fun, q);

    // Free func_flags to save memory
    q->func_flags = NULL;
    ck_free(q->func_flags);

    if (unlikely(fun->logmode)) {
      fprintf(fun->log_fp, "compute_sig_score_for_queue(), "
              "min_score %lf, max_score %lf, q_score %lf\n",
              fun->min_score, fun->max_score, q->sig_score);
    }
...
/// Preserve sig_scores of queued seeds at the end of a successful funfuzz
void record_sigdata_for_queue(afl_state_t *afl) {

  u8    *fpath  = NULL;
  FILE  *fp     = NULL;

  // Open file
  fpath = alloc_printf("%s/q_sigdata", afl->out_dir);
  fp    = fopen(fpath, "w");
  if (!fp) {
    // Sanitize. Deinitialize fun in advance.
    fun_deinit(afl);
    FATAL("record_qscores(), fopen(qscores_file) failed!");
  }

  // Log
  FUN_LOG("Record fs info for queued cases to '%s'...", fpath);

  // Write sigdata in format: <id>,<sig_score>,<bitmap_size>,<favored>
  fprintf(fp, "id,sig_score,bitmap_size,favored\n");
  for (u32 i = 0; i < afl->queued_items; ++i) {
    struct queue_entry *q = afl->queue_buf[i];
    fprintf(fp, "%u,%lf,%u,%d\n",
            q->id, q->sig_score, q->bitmap_size, q->favored);
  }
