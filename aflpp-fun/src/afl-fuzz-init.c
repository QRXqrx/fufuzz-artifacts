...
// Amazing, the order of #include can affect compilations.
#include "afl-fuzz.h"
#include <limits.h>
#include "cmplog.h"
...
// @FUN: preserve flags for executed functions
void perform_dry_run(afl_state_t *afl) {
  ...
  // perform_dry_run() -> calibrate_case() -> fuzz_run_target() -> afl_fsrv_run_target()
  for (idx = 0; idx < afl->queued_items; idx++) {
    ...
    // calibrate_case() -> fuzz_run_target() -> afl_fsrv_run_target()
    res = calibrate_case(afl, q, use_mem, 0, 1);

    // @FUN: preserve func_flags for initial test cases.

    q->func_flags = (u8 *) malloc(fun->fs_len * sizeof(u8));

    if (fun->on) {

      // Copy function flags from currently hot cc_shm.
      for (u32 i = 1; i < fun->fs_len; ++i)
        q->func_flags[i] = (u8) fun->cc_shm[CC_IDX_HOT][i];
...
void setup_testcase_shmem(afl_state_t *afl) {
  ...
  // Debug
  FUN_LOG("`afl_shm_init()` in `afl-fuzz-init.h#setup_testcase_shmem`!");
...
/// Read funcInfo file to get the number of functions we have.
/// @param tmpdir: AFL_FUN_TEMP
u32 get_func_num(u8 *tmpdir) {

  const u8 *FUNC_INFO = "funcInfo";
  ...
  fpath = alloc_printf("%s%c%s", tmpdir, file_sep(), FUNC_INFO);
  FUN_LOG("get_func_num(), funcInfo '%s'", fpath);
  ...
  // Sanitize
  if (fp == NULL) {
    FATAL("get_func_num(), Cannot open funcInfo file `%s`!", fpath);
  }
...
/// Read staticFS from FUN_TEMP_DIR.
double *read_static_fs(fun_globals_t *fun) {

  // Defensive programming.
  if (!fun->fs_static) return NULL;

  const u8 *STATIC_FS = "staticFS";

  // Declarations.

  u8 *fpath;
  u32 len1d_fs;     /* fs_val_num + 1 */

  // Locate static fs file.

  fpath = alloc_printf("%s%c%s", fun->tmpdir, file_sep(), STATIC_FS);
  FUN_LOG("staticFS file '%s'", fpath);

  // Sanitize

  len1d_fs = get_func_num(fun->tmpdir) + 1;
  if (fun->fs_len != len1d_fs) {
    FATAL("Invalid staticFS file, len1d(staticFS) %d, len1d(fs_shm) %d",
          len1d_fs, fun->fs_len);
  }

  // Read in static FS values

  FILE   *fp;
  double *fs_shm_static;
  char *  line      = NULL;
  size_t  read_size = 0;
  int     cnt       = 0;
  ...
  if (fp == NULL) FATAL("Invalid staticFS file `%s`", fpath);

  fs_shm_static = (double *)malloc(fun->fs_shm_size);

  while (getline(&line, &read_size, fp) != -1) {

    // Functions are indexed from 1
    fs_shm_static[++cnt] = strtod(line, NULL);

  }

  return fs_shm_static;

}

/// Essentially a refined popen that able to trace the pid. This function
/// attach stdin or stdout to given file descriptors, execute dcc script,
/// and return the pid of running dcc.
/// @param afl: the running afl instance.
/// @param infdptr: pointer to the fd used to attach to subprocess' stdin,
///                 NULL for unused.
/// @param outfdptr: pointer to the fd used to attach to subprocess' stdout,
///                  NULL for unused.
/// @return pid of dcc process.
pid_t fun_dcc_init(afl_state_t *afl, s32 *infdptr, s32 *outfdptr) {
...
#define WT 1   /* The pipe position prepared for writing */
  ...
  // Validate dcc_path
  dcc_path = getenv("FUN_DCC");
  if (!dcc_path) FATAL("fun_dcc_init(), FUN_DCC is empty!");
  ...
  if (pid < 0) {
    ...
    */
    ...
    // Start dcc process
    execlp("python3", /* Program responsible for this execution */
           "python3", dcc_path, afl->fun.tmpdir, afl->out_dir, NULL);

    PFATAL("fun_dcc_init(), execlp() failed");
...
void fun_init(afl_state_t *afl, u8 *tmpdir) {

  fun_globals_t *fun      = &afl->fun;
  sharedmem_t   *shm      = &afl->shm;

  u8   *log_path = NULL;
  s32   infd     = -1;   /* Temporary file descriptor to DCC stdin */

  ACTF("Setting up fun mode...");

  // Configure @FUN mode

  afl->shm.fun_mode = 1;

  if (!getenv("FUN_SCHEDULE_OFF")) {

    // Sometimes we may not want a FUN schedule under fun mode.
    afl->schedule = FUN;
    FUN_LOG("OK, FUN-schedule is on to bring more fun!");

  } else {

    FUN_LOG("Whoops, FUN-schedule is off...");
  ...
  fun->fuzz_started = 0;
  fun->tmpdir       = tmpdir;
  fun->func_num     = get_func_num(fun->tmpdir);

  // Log mode

  fun->logmode = !!getenv("FUN_LOG");

  if (unlikely(fun->logmode)) {

    FUN_LOG("Log mode on!");

    log_path    = alloc_printf("%s/funlog", afl->out_dir);
    fun->log_fp = fopen(log_path, "w");
    if (!fun->log_fp) FATAL("fun_init(), fopen(log_file) failed!");
    ck_free(log_path);
  ...
  fun->fs_static    = !!getenv("FUN_STATIC_FS");  // TODO: do more according to the env value?
  fun->fs_len       = fun->func_num + 1;
  fun->fs_shm_size  = fun->fs_len * FS_BYTES;

  // Initially, we activate No.0 fs_shm as hot.
  fun->fs_idx_hot = 0;
  fun->fs_idx_cld = 1;

  if (fun->fs_static) {

    // Use static FS values.
    FUN_LOG("Well, seems the user wants to use static FS...");
    afl->power_name = alloc_printf("%s-static", power_names[afl->schedule]);

    // Store fs vals to cold fs_shm
    fun->fs_shm[0] = NULL;
    fun->fs_shm[1] = read_static_fs(&afl->fun);

  } else {

    // Use dynamic FS values. Dual-SHM and DCC process are required
    FUN_LOG("Dynamic fs mode! Yeah!");
    afl->power_name = alloc_printf("%s-dynamic", power_names[afl->schedule]);

    // Start dcc process
    infd = -1;
    fun->dcc_pid  = fun_dcc_init(afl, &infd, NULL);
    if (infd == -1) {
      kill(fun->dcc_pid, SIGTERM);
      FATAL("fun_init(), invalid infd (%d)", infd);
    }
    fun->dcc_infp = fdopen(infd, "w");
    if (!fun->dcc_infp) {
      kill(fun->dcc_pid, SIGTERM);
      FATAL("fun_init(), fdopen(dcc_infp) failed");
    }

    // Map and attach 2 fs_shm
    for (u8 idx = 0 ; idx < SHM_IDX; ++idx)
      fun->fs_shm[idx] = fun_fs_shm_init(shm, fun->fs_shm_size, idx);

  }

  // CC SHMs

  fun->cc_len        = fun->fs_len * fun->fs_len;  /* 2d for FS */
  fun->cc_shm_size   = fun->cc_len * CC_BYTES;
  fun->cc_flags_size = fun->func_num * CC_BYTES;
  for (u8 idx = 0 ; idx < SHM_IDX; ++idx)
    fun->cc_shm[idx] = fun_cc_shm_init(shm, fun->cc_shm_size, idx);

  // Configure fun scheduling.

  // Initialize min and max.
  fun->min_score              = DBL_MAX;
  fun->max_score              = DBL_MIN; /* The smallest positive double */
  fun->total_sig_score        = 0.0;

  afl->min_bitmap_size        = INT32_MAX;
  afl->max_bitmap_size        = 0;

  // Set update period
  fun->fs_update_period = 60;         /* 60s as default         */
  if (getenv("FUN_UPDATE_PERIOD"))    /* Allow config from env  */
    fun->fs_update_period = strtoll(getenv("FUN_UPDATE_PERIOD"), NULL, 10);

}

void fun_deinit(afl_state_t *afl) {

  fun_globals_t *fun = &afl->fun;
  sharedmem_t   *shm = &afl->shm;

  ACTF("Not fun anymore, destroy it");

  // Release SHMs.
  fun_shm_deinit(shm);
  for (u8 i = 0; i < SHM_IDX; ++i) {
    fun->cc_shm[i] = NULL;
    fun->fs_shm[i] = NULL;
  }

  // Kill DCC process in dynamic mode

  if (!fun->fs_static) {
    FUN_LOG("Kill DCC process...");
    if (!(waitpid(fun->dcc_pid, NULL, WNOHANG) == -1)) {
      kill(fun->dcc_pid, SIGTERM);
    }

    // Close file pointer.
    FUN_LOG("Close infp to dcc process...");
    fclose(fun->dcc_infp);
  }
